...
export default function JoinCaptainForm() {
  ...
  <Select
  value={form.vehicleType || (type ? `${type} Seater` : "")}
  onChange={e=>set("vehicleType", e.target.value)}
>
  <option value="">Select vehicle type…</option>

  {type ? (
    <option>{type} Seater</option>
  ) : (
...
{/* ✅ CAB TYPE (ONLY SHOW FOR CABS) */}
{["4 Seater","6 Seater","8 Seater"].includes(form.vehicleType) && (
  <div>
    <Label required>Cab Category</Label>
    <Select onChange={(e)=>setCabType(e.target.value)}>
      <option value="">Select cab category</option>
      <option>4 Seater (Sedan)</option>
      <option>6 Seater (SUV)</option>
      <option>8 Seater (Innova / Traveller)</option>
    </Select>
  </div>
)}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
